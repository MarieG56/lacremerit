...
function getOrderItemUnit(item: any): string {
    if (item.unit && item.unit.trim() !== "") {
        return item.unit;
    }
    if (item.product && (item.product as Product).unit && (item.product as Product).unit.trim() !== "") {
        return (item.product as Product).unit;
    }
    return "KG";
}
        ...
        setEditValues({
            ...
            orderItems: (order.orderItems ?? []).map((item) => ({
                ...
                unit: item.unit && item.unit.trim() !== ""
                    ? item.unit
                    : (item.product?.unit && (item.product as Product).unit.trim() !== ""
                        ? (item.product as Product).unit
                        : "KG"),
    ...
    async function saveEdit(order: Order) {
        ...
        try {
            ...
            await updateOrder(order.id, {
                ...
                customerId: typeof editValues?.customerId === "number" ? editValues.customerId : undefined,
                totalAmount,
            });

            const newItems =
                editValues?.orderItems?.filter((item) => !item.id || item.id < 0) ?? [];
            const existingItems =
                editValues?.orderItems?.filter(
                    (item) =>
                        item.id && item.id > 0 && !deletedOrderItemIds.includes(item.id)
                ) ?? [];
        ...
        <div className="min-h-screen bg-gray-100 w-full max-w-full mx-auto">
            <header className="bg-white shadow-md p-4">
                <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between px-1 md:px-3">
                    <h1 className="text-2xl font-bold flex-1 text-center md:text-left">Commandes</h1>
            ...
            <main className="p-4 md:p-6">
                ...
                {/* Affichage du récapitulatif par producteur */}
                <div className="mb-8">
                            ...
                            {(() => {
                                ...
                                ordersThisWeek.forEach((order) => {
                                    order.orderItems?.forEach((item) => {
                                        ...
                                        const productKey = product?.name || "Produit inconnu";
                                ...
                                if (producers.length === 0) {
                                    return <div className="text-gray-400 text-center">Aucune commande cette semaine.</div>;
                                }
                                return (
                                    <div className="flex flex-wrap gap-4 md:gap-8 justify-center md:justify-start">
                                        {producers.map((producer) => (
                                            <div key={producer} className="mb-4 min-w-[180px]">
                                                <div className="font-semibold mb-2">{producer}</div>
                                                <ul className="list-none pl-0">
                                                    {Object.values(recap[producer])
                                                        .sort((a, b) => a.productName.localeCompare(b.productName))
                                                        .map((prod) => (
                                                            <li key={prod.productName}>
                                                                <span className="font-bold">{prod.quantity}</span> {prod.productName}
                                                            </li>
                                                        ))}
                                                </ul>
                                            </div>
                                            ...
                                            <td className="px-2 py-2 md:px-4 md:py-2 text-center align-top">
                                                {editingOrderId === order.id ? (
                                                    <select
                                                        ...
                                                        onChange={(e) =>
                                                            setEditValues((prev) => ({ ...prev!, customerId: Number(e.target.value) }))
                                                        }
                                                        disabled={saving}
                                                    >
                                                    ...
                                                    <ul>
                                                        {order.orderItems?.map((item) => (
                                                            <li key={item.id}>
                                                                ...
                                                                {item.product?.name} {getOrderItemUnit(item)} ({item.unitPrice} €)
                                                            </li>
                                                        ))}
                                                    </ul>
                                                )}
                                                ...
                                                <tr className="bg-gray-50">
                                                    <td colSpan={7} className="px-2 py-2">
                                                        ...
                                                        <span className="block text-base font-semibold">
                                                            {(editValues?.orderItems.reduce((sum, item) => sum + item.quantity * item.unitPrice, 0) ?? 0).toFixed(2)} €
                                                        </span>
                                                    </td>
                                                </tr>
                                            </>
